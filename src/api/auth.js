...
export async function createUserToken(userObj) {
  try {
    ...
    logger.debug(`createUserToken() jwtToken: ${jwtToken}`);
    return jwtToken;
  } catch (error) {
...
export async function getTokenByWechatScan(code, state) {
  try {
    ...
    logger.debug('code %j, state %j', code, state);

    const qyUserObj = await qydev.getUserWithDepartments(code);
    logger.debug('getTokenByWechatScan() user: %j', qyUserObj);
